...
**Follow-up note (bit scheme corrected):** the guess above was wrong. External reference material was later obtained and corroborated by three independent sources (PICO-8's own reference C source `pxa_compress_snippets.c`, the independent Python reimplementation shrinko8, and pancelor's reverse-engineering blog post — see `docs/prd12-research-findings.md`), which agree exactly on every bit-level detail. `decodeLua`'s MTF-index encoding is a bucketed scheme (fixed width `4+n`, bucket base `16*(2^n-1)`, not Elias-gamma), and the offset-width selector is variable-length (1 bit for 15-bit width, 2 bits for 10-bit/5-bit), not a fixed 2 bits. With the fix applied, decoded output from real fixtures is genuine readable Lua source (function/local/end/if/then keywords, `--` comments, e.g. `cab ride.p8.png` decodes to `-- cab ride\n-- by ben jones/@powersaurus\n...`).
