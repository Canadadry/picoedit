...
The Music tab (`app/music/MusicTab.tsx`) shows all 64 patterns (`CartData.music`, indices 0-63) as a vertically scrollable list of rows. Each row is one pattern and shows:
...
- Reuses `CartContext` from PRD 18 (`app/state/CartContext.tsx`) via `updateCart({ music: ... })` on every edit — same pattern every other section tab uses.
- Follows the established folder shape: `app/music/MusicTab.tsx` + `app/music/components/` (e.g. a `PatternRow.tsx`, `ChannelCell.tsx`).

## Testing Decisions

Per PRD 18's precedent, no automated component tests are added (framework choice deferred to the code-organization discussion). Manual verification: load a real fixture from `cart/`, confirm all 64 patterns render with their actual decoded values, edit a channel's sfx id and a loop flag, download, and re-decode the result via `cmd/cli.ts` to confirm the edited pattern round-trips correctly through `encodeMusic`/`decodeMusic` (already bit-exact per `docs/prd/done/08-music.md`).
