...
`CartData.gff` (`SpriteFlags[]`, 256 entries of 8 named booleans — `internal/pico8/cart-gff.ts`, PRD 04, already `done`) can currently only be inspected or changed by hand-editing `gff.json`. Depends on PRD 18 (app shell/`CartContext`), PRD 16 (repo restructure), and PRD 25 (library fix: full 256-sprite `cart.gfx.pixels`, used here only for thumbnail rendering) landing first.
...
- **Flag colors are cosmetic only, not data**: `SpriteFlags` has no color field — PICO-8's colored dots are a fixed UI convention, not part of `CartData`. This tab reuses `internal/pico8/palette.ts`'s `PICO8_PALETTE` indices 8-15 (in flag 0-7 order) as that same fixed convention, purely for the toggle dots' color — a judgment call since the exact native default mapping wasn't confirmed from public docs, but it's non-load-bearing (no data implication either way).
...
- **Thumbnail rendering**: this tab only reads sprite pixels (for thumbnails), never writes them — so with PRD 25 landed, it's a plain `cart.gfx.pixels` lookup for all 256 sprites, no helper needed. (PRD 19's `app/state/shared-sprite-region.ts` write-mirroring helper is irrelevant here since GFF never edits pixels.)

## Testing Decisions

- No React component test framework yet (same deferral as PRD 18/19). Manual verification: load a real fixture, toggle flags via both the grid and the detail panel, download, and confirm `gff.json`-equivalent state round-trips (e.g. by decoding the exported cart again and diffing).
- `spriteFlagsToByte`/`byteToSpriteFlags`-equivalent logic already has unit + integration tests in `internal/pico8/cart-gff.test.ts` (PRD 04) — this PRD adds no new binary-layout logic, only UI, so no new tests are needed for the data layer itself.
...
- Any change to `internal/pico8/cart-gff.ts`'s existing decode/encode scope.
